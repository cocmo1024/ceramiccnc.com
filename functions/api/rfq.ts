...
const DEFAULT_FROM_EMAIL = 'rfq@ceramiccnc.com';
...
const allowedApplications = new Set([
  'Precision ceramic structural part',
  'Plunger, pump, or dosing component',
  'Dispensing valve or fluid-control part',
  'Tube, ring, shaft, sleeve, rod, or pin',
  'Plate, sheet, substrate, or fixture',
  'Vacuum pickup or semiconductor component',
  'Electrical, thermal, laser, or insulating component',
  'Medical, food-equipment, or wear component',
  'Other ceramic part',
]);
...
export const onRequestPost: WorkerHandler = async ({ request, env }) => {
  ...
  const subject = `[Ceramic RFQ] ${safeSubjectCompany} — ${safeSubjectApplication}`;
  ...
  try {
    emailResult = await env.RFQ_EMAIL.send({
      ...
      html: `<h1 style="font-size:20px">New precision ceramic RFQ</h1><table>${htmlDetails}</table><h2 style="font-size:16px;margin-top:24px">Project requirements</h2><p style="white-space:pre-wrap">${escapeHtml(requirements)}</p>`,
      attachments,
      headers: {
        'X-CeramicCNC-Submission-ID': submissionId,
      },
    });
  } catch (error) {
